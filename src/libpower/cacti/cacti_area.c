...
double calculate_area(module_area,techscaling_factor)
area_type module_area;
double techscaling_factor;
{
	return(module_area.height*module_area.width*(1/techscaling_factor)*(1/techscaling_factor));
}

area_type inverter_area(Widthp, Widthn)
double Widthp, Widthn;
{
	double Width_n,Width_p;
	area_type invarea;
	int foldp=0, foldn=0;
	if (Widthp>10.0) { Widthp=Widthp/2, foldp=1;}
	if (Widthn>10.0) { Widthn=Widthn/2, foldn=1;}
...
area_type subarraymem_area(C,B,A,Ndbl,Ndwl,Nspd,RWP,ERP,EWP,NSER,techscaling_factor)/* returns area of subarray */
int C, B, A, Ndbl, Ndwl, Nspd ,RWP,ERP,EWP,NSER;
double techscaling_factor;

{
	...
	area_all_dataramcells=Ndwl*Ndbl*calculate_area(memarea,techscaling_factor)/100000000.0;
	return(memarea);
}

area_type decodemem_row(C,B,A,Ndbl,Nspd,Ndwl,RWP,ERP,EWP) /* returns area of post decode */ 
int C,B,A,Ndbl,Ndwl,Nspd,RWP,ERP,EWP;
{
	...
	desiredrisetime = krise*log((double)(noof_colns))/2.0;
        ...
        numstack =
          ceil((1.0/3.0)*logtwo_area( (double)((double)C/(double)(B*A*Ndbl*Nspd))));
...
area_type predecode_area(noof_rows,RWP,ERP,EWP) /*returns the area of predecode */
int noof_rows,RWP,ERP,EWP;
...
area_type postdecode_area(noof_rows,RWP,ERP,EWP)
int noof_rows, RWP,ERP,EWP;
{ 
...
area_type colmux(Ndbl, Nspd, RWP, ERP, EWP, NSER) /* gives the height of the colmux */
int Ndbl, Nspd, RWP, ERP, EWP, NSER;
{ 
...
area_type precharge(Ndbl,Nspd,RWP,ERP,EWP,NSER)
int Ndbl, Nspd, RWP,ERP,EWP,NSER;
{
...
area_type senseamp(Ndbl,Nspd,RWP,ERP,EWP,NSER)
int Ndbl,Nspd, RWP,ERP,EWP,NSER;
{
...
area_type subarraytag_area(baddr,C,B,A,Ntdbl,Ntdwl,Ntspd,NSubbanks,RWP,ERP,EWP,NSER,techscaling_factor) /* returns area of subarray */
int baddr, C, B, A, Ntdbl, Ntdwl, Ntspd,RWP,ERP,EWP,NSER;
double techscaling_factor;
double NSubbanks;

{
	...
	noof_rows = (C/(B*A*Ntdbl*Ntspd));
        ...
        area_all_tagramcells=Ntdwl*Ntdbl*calculate_area(tagarea,techscaling_factor)/100000000.0;
        return(tagarea);
}

area_type decodetag_row(baddr,C,B,A,Ntdbl,Ntspd,Ntdwl,NSubbanks,RWP,ERP,EWP) /* returns area of post decode */
int baddr,C,B,A,Ntdbl,Ntdwl,Ntspd,RWP,ERP,EWP;
double NSubbanks;
{
        int noof_colns,numstack, Tagbits;
	double decodeNORwidth;
        area_type decinv,worddriveinv,postdecodearea;

        Tagbits = baddr-logtwo_area((double)(C))+logtwo_area((double)(A))+2-logtwo_area((double)(NSubbanks));
       
	numstack =
          ceil((1.0/3.0)*logtwo_area( (double)((double)C/(double)(B*A*Ntdbl*Ntspd))));
        if (numstack==0) numstack = 1;
        if (numstack>5) numstack = 5;
        switch(numstack) {
          case 1: decodeNORwidth = WidthNOR1; break;
...
area_type comparatorbit(RWP,ERP,EWP)
int RWP,ERP,EWP;
{
...
area_type muxdriverdecode(B, b0, RWP, ERP, EWP)
int B, b0, RWP, ERP, EWP;
...
area_type muxdrvsig(A,B,b0) /* generates the 8B/b0*A signals */
int A, B, b0;
{
	int noof_rows;
...
area_type datasubarray(C,B,A,Ndbl,Ndwl,Nspd,RWP,ERP,EWP,NSER,techscaling_factor)
int C,B,A,Ndbl,Ndwl,Nspd,RWP,ERP,EWP,NSER;
double techscaling_factor;
{

	area_type datasubarray_area,mem_area,postdecode_area,colmux_area,precharge_area,senseamp_area,outdrv_area;
...
area_type datasubblock(C,B,A,Ndbl,Ndwl,Nspd,SB,b0,RWP,ERP,EWP,NSER,techscaling_factor)
int C,B,A,Ndbl,Ndwl,Nspd,SB,b0,RWP,ERP,EWP,NSER;
double techscaling_factor;

{
	...
	N3to8=ceil((1.0/3.0)*logtwo_area( (double)(C/(B*A*Ndbl*Nspd))));
        if (N3to8==0) {N3to8=1;}
...
area_type dataarray(C,B,A,Ndbl,Ndwl,Nspd,b0,RWP,ERP,EWP,NSER,techscaling_factor)
int C,B,A,Ndbl,Ndwl,Nspd,b0,RWP,ERP,EWP,NSER;
double techscaling_factor;
{
	int SB, N3to8;
	...
	double inter_height,inter_width, total_height=0.0, total_width=0.0;
	...
	datasubblock_area=datasubblock(C,B,A,Ndbl,Ndwl,Nspd,SB,b0,RWP,ERP,EWP,NSER,techscaling_factor);
        area_all_datasubarrays=Ndbl*Ndwl*calculate_area(datasubarray_area,techscaling_factor)/100000000.0;
...
area_type tagsubarray(baddr,C,B,A,Ndbl,Ndwl,Nspd,NSubbanks,RWP,ERP,EWP,NSER,techscaling_factor)
int baddr,C,B,A,Ndbl,Ndwl,Nspd,RWP,ERP,EWP,NSER;
double techscaling_factor;
double NSubbanks;
{
	int conservative_NSER;
...
area_type tagsubblock(baddr,C,B,A,Ndbl,Ndwl,Nspd,NSubbanks,SB,RWP,ERP,EWP,NSER,techscaling_factor)
int baddr,C,B,A,Ndbl,Ndwl,Nspd,SB,RWP,ERP,EWP,NSER;
double techscaling_factor;
double NSubbanks;
{	
	int N3to8, T;
	...
	T= baddr-logtwo_area((double)(C))+logtwo_area((double)(A))+2-logtwo_area((double)(NSubbanks));
...
area_type tagarray(baddr,C,B,A,Ndbl,Ndwl,Nspd,NSubbanks,RWP,ERP,EWP,NSER,techscaling_factor)
int baddr,C,B,A,Ndbl,Ndwl,Nspd,RWP,ERP,EWP,NSER;
double techscaling_factor;
double NSubbanks;
{
	...
	tagsubarray_area=tagsubarray(baddr,C,B,A,Ndbl,Ndwl,Nspd,NSubbanks,RWP,ERP,EWP,conservative_NSER,techscaling_factor);
        ...
        area_all_tagsubarrays=Ndbl*Ndwl*calculate_area(tagsubarray_area,techscaling_factor)/100000000.0;
...
void area(baddr,b0,Ndbl,Ndwl,Nspd,Ntbl,Ntwl,Ntspd,NSubbanks,parameters,result)
int baddr,b0,Ndbl,Ndwl,Nspd,Ntbl,Ntwl,Ntspd;
...
double NSubbanks;
{
	...
	result->totalarea=calculate_area(result->dataarray_area,parameters->fudgefactor)+calculate_area(result->datapredecode_area,parameters->fudgefactor)+calculate_area(result->datacolmuxpredecode_area,parameters->fudgefactor)+calculate_area(result->datacolmuxpostdecode_area,parameters->fudgefactor)+(parameters->num_readwrite_ports+parameters->num_write_ports)*calculate_area(result->datawritesig_area,parameters->fudgefactor)+calculate_area(result->tagarray_area,parameters->fudgefactor)+calculate_area(result->tagpredecode_area,parameters->fudgefactor)+calculate_area(result->tagcolmuxpredecode_area,parameters->fudgefactor)+calculate_area(result->tagcolmuxpostdecode_area,parameters->fudgefactor)+calculate_area(result->tagoutdrvdecode_area,parameters->fudgefactor)+(parameters->num_readwrite_ports+parameters->num_read_ports)*calculate_area(result->tagoutdrvsig_area,parameters->fudgefactor);

}

area_type fadecode_row(C,B,Ndbl,RWP,ERP,EWP) /*returns area of post decode */
int C,B,Ndbl,RWP,ERP,EWP;
{
        int numstack;
...
area_type fasubarray(baddr,C,B,Ndbl,RWP,ERP,EWP,NSER,techscaling_factor)/* returns area of subarray */
int baddr, C, B, Ndbl,RWP,ERP,EWP,NSER;
double techscaling_factor;

{
        ...
        double precharge, widthoverhead=0.0, heightoverhead=0.0;
        ...
        faramcell.height=ceil((double)(noof_rowsdata)/16.0)*stitch_ramv+(CAM2x2Height_1p+2*heightoverhead)*ceil((double)(noof_rowsdata)/2.0);

        faramcell.width=(ceil((double)(noof_colnsdata)/16.0))*(BitWidth16x2+16*(Widthtrack*2*(RWP+(ERP-NSER)+EWP-1)+Widthtrack*NSER))+2*(HTagbits*((CAM2x2Width_1p+2*widthoverhead)-Widthcontact))+(BitWidth+Widthtrack*2*(RWP+ERP+EWP-1))+(FArowNANDWidth+FArowNOR_INVWidth)*(RWP+ERP+EWP);
        ...
        area_all_dataramcells=Ndbl*calculate_area(faramcell,techscaling_factor)/100000000.0;
        faarea_all_subarrays=Ndbl*calculate_area(FAarea,techscaling_factor)/100000000.0;
        return(FAarea);
}

area_type faarea(baddr,b0,C,B,Ndbl,RWP,ERP,EWP,NSER,techscaling_factor)
int baddr,b0,C,B,Ndbl,RWP,ERP,EWP,NSER;
double techscaling_factor;
{
...
void fatotalarea(baddr,b0,Ndbl,parameters,faresult)
int baddr,b0,Ndbl;
parameter_type *parameters;
...

{
        ...
        faresult->totalarea=(calculate_area(faresult->dataarray_area,parameters->fudgefactor)+calculate_area(faresult->datapredecode_area,parameters->fudgefactor)+calculate_area(faresult->datacolmuxpredecode_area,parameters->fudgefactor)+calculate_area(faresult->datacolmuxpostdecode_area,parameters->fudgefactor)+calculate_area(faresult->tagarray_area,parameters->fudgefactor)+calculate_area(faresult->tagpredecode_area,parameters->fudgefactor)+calculate_area(faresult->tagcolmuxpredecode_area,parameters->fudgefactor)+calculate_area(faresult->tagcolmuxpostdecode_area,parameters->fudgefactor)+calculate_area(faresult->tagoutdrvdecode_area,parameters->fudgefactor)+calculate_area(faresult->tagoutdrvsig_area,parameters->fudgefactor));

}


void area_subbanked(baddr,b0,RWP,ERP,EWP,Ndbl,Ndwl,Nspd,Ntbl,Ntwl,Ntspd,NSubbanks,parameters,result_subbanked,result)
int baddr,b0,RWP,ERP,EWP,Ndbl,Ndwl,Nspd,Ntbl,Ntwl,Ntspd;
double NSubbanks;
...
parameter_type *parameters;
{
	...
	double base_height,base_width,inter_width,inter_height,total_height=0.0,total_width=0.0;
	...
	int i,iter_height,iter_width,iter_width_double;
      ...
      aspect_ratio_total = 1.0;
        ...
        area(baddr,b0,Ndbl,Ndwl,Nspd,Ntbl,Ntwl,Ntspd,NSubbanks,parameters,&result_area); }
        else {
	...
	if (NSubbanks == 1) {
        ...
        if (NSubbanks == 2) {
                ...
                total_width = (result_area.dataarray_area.width+result_area.tagarray_area.width)*2 + (ADDRESS_BITS+BITOUT)*NSubbanks*(RWP+ERP+EWP);
	}
        if (NSubbanks == 4) {
        ...
        if (NSubbanks == 8) {
                total_height = (result_area.dataarray_area.width+result_area.tagarray_area.width)*2 + (ADDRESS_BITS+BITOUT)*NSubbanks*(RWP+ERP+EWP)*0.5;
                total_width = 2*(2*result_area.dataarray_area.height+2*(RWP+ERP+EWP)*ADDRESS_BITS)+(ADDRESS_BITS+BITOUT)*NSubbanks*(RWP+ERP+EWP);
	}
	...
	if (NSubbanks > 8 ) {
	 ...
	 iter_width= 10;
	  ...
	  if ( htree % 2 == 0) { 
		...
		total_width+=(ADDRESS_BITS+BITOUT)*(RWP+ERP+EWP)*NSubbanks/2;
	  }
	 }	
	}
...
int organizational_parameters_valid(B,A,C,Ndwl,Ndbl,Nspd,Ntwl,Ntbl,Ntspd,assoc)
int B,A,C,Ndwl,Ndbl,Nspd,Ntwl,Ntbl,Ntspd;
char assoc;
{
