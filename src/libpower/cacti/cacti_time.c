...
int cols_data_subarray,rows_data_subarray,cols_tag_subarray,rows_tag_subarray;
double subbank_v,subbank_h, sub_v=0.0,sub_h=0.0,inter_v,inter_h,htree;
int htree_int,tagbits;
int cols_fa_subarray,rows_fa_subarray;
  ...
  if(!fullyassoc) {
	     ...
	     subbank_h = 0;
	     subbank_v = 0;
	     single_senseext_driver=senseext_driver_delay(C,B,A,fullyassoc,Ndbl,Nspd,Ndwl,Ntbl,Ntspd,inputtime,outputtime, subbank_v, subbank_h, &single_power);
...
{
   double Ceq,Tst1,tf;
   int rows,tagbits,cols,htree_int,l_valdrv_v=0,l_valdrv_h=0,exp;
   double wire_cap,wire_res;
   double htree;
   double subbank_v,subbank_h;
...
/*======================================================================*/
extern int organizational_parameters_valid(int B, int A, int C,
    int Ndwl, int Ndbl,int Nspd,int Ntwl, int Ntbl,int Ntspd,char assoc);
...
int useTag;
{
   ...
   int l_muxdrv_v=0, l_muxdrv_h=0, exp, htree_int;
	...
	for (Ntbl=1;Ntbl<=MAXN;Ntbl=Ntbl*2) {
	  if (organizational_parameters_valid
	    ...
	    total_address_routing_power*=(parameters->num_readwrite_ports+parameters->num_read_ports+parameters->num_write_ports);
            ...
            if(*NSubbanks > 2 ) {
	    subbankaddress_routing_delay= address_routing_delay(parameters->cache_size, parameters->block_size, parameters->associativity, parameters->fully_assoc, Ndwl, Ndbl, Nspd, Ntwl, Ntbl,Ntspd,NSubbanks,&outrisetime,&subbankaddress_routing_power);
	    }
	    ...
	    bitline_data = bitline_delay(parameters->cache_size,parameters->associativity,
					 ...
					 inrisetime,&outrisetime, &bitline_data_power);
...

	    {
	      ...
	      sense_amp_data = sense_amp_delay(inrisetime,&outrisetime, temp_row, &sense_amp_data_power);
	    ...
	    if(*NSubbanks > 2 ) {
		total_out_driver = senseext_driver_delay(parameters->cache_size,parameters->block_size,parameters->associativity,parameters->fully_assoc,Ndbl,Nspd,Ndwl,Ntbl,Ntspd,inrisetime,&outrisetime, subbank_v, subbank_h, &total_out_driver_power);
	    }
...
*/
	    ...
	    total_power=(subbankaddress_routing_power+decoder_data_power+wordline_data_power+bitline_data_power+sense_amp_data_power+total_out_driver_power+decoder_tag_power+wordline_tag_power+bitline_tag_power+sense_amp_tag_power+compare_tag_power+valid_driver_power+mux_driver_power+selb_power+data_output_power)/FUDGEFACTOR;	  
            ...
            total_power_without_routing=(*NSubbanks)*(decoder_data_power+wordline_data_power+bitline_data_power+sense_amp_data_power+decoder_tag_power+wordline_tag_power+bitline_tag_power+sense_amp_tag_power+compare_tag_power+mux_driver_power+selb_power+data_output_power)/FUDGEFACTOR+valid_driver_power/FUDGEFACTOR;
...
*/
	 ...
	 total_power=(subbankaddress_routing_power+decoder_data_power+wordline_data_power+bitline_data_power+sense_amp_data_power+data_output_power+total_out_driver_power)/FUDGEFACTOR;	  
            ...
            total_power_without_routing=(*NSubbanks)*(decoder_data_power+wordline_data_power+bitline_data_power+sense_amp_data_power+data_output_power)/FUDGEFACTOR;
