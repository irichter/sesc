...
class ThreadContext {
 ...
 private:
  ...
  // Descriptor of the next instruction to execute
  InstDesc *nextInstDesc;
  // Descriptor of the instruction we will jump to after the delay slot executes
  InstDesc *jumpInstDesc;
...
public:
 ...
 tls::Epoch *getEpoch(void) const{
...
#if (defined MIPS_EMUL)
  inline InstDesc *virt2inst(VAddr vaddr) const{
    return addressSpace->virtToInst(vaddr);
  }
#else // For (defined MIPS_EMUL)
...
#if (defined MIPS_EMUL)
  inline InstDesc *getJumpInstDesc(void) const {
    return jumpInstDesc;
  }
  inline void setJumpInstDesc(InstDesc *instDesc){
    jumpInstDesc=instDesc;
  }
  inline InstDesc *getNextInstDesc(void) const {
    return nextInstDesc;
  }
  inline void setNextInstDesc(InstDesc *instDesc){
    nextInstDesc=instDesc;
  }
  inline VAddr getLastDataVAddr(void) const {
    return lastDataVAddr;
  }
  inline void setLastDataVAddr(VAddr dVAddr){
    lastDataVAddr=dVAddr;
  }
  ...
  void    writeMemFromBuf(VAddr addr, size_t len, void *buf);
  ssize_t writeMemFromFile(VAddr addr, size_t len, int fd);
  void    writeMemWithByte(VAddr addr, size_t len, uint8_t c);  
  void    readMemToBuf(VAddr addr, size_t len, void *buf);
  ssize_t readMemToFile(VAddr addr, size_t len, int fd);
  ...
  inline bool readMem(VAddr addr, T &val){
    bool retVal=addressSpace->readMemRaw<T>(addr,val);
  ...
  inline bool writeMem(VAddr addr, const T &val){
    T tmp=val;
    cvtEndianBig(tmp);    
    return addressSpace->writeMemRaw<T>(addr,tmp);
  }
