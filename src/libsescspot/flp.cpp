...
// File: flp.cpp
//
// Description: This file reads the floorplan description file and generates
// a list of elements with the dimensions for the power simulation
//             
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
// print_flp_fig : Takes the foorplan info and dumps it to stderr 
//                 same format as the specified in the .flp header
...
void print_flp_fig (flp_t *flp)
{
  ...
  fprintf(stderr, "FLP: Generating FIG from floorplan file\n");
  fprintf(stderr, "FLP: FIG starts\n");
...
flp_t *read_flp(const char *file)
{
  ...
  if (!fp) 
    {
      sprintf(str, "FLP: Error opening file %s\n", file);
      fatal(str);
    }
  ...
  if(!count)
    fatal("FLP: No units specified in the floorplan file\n");
  
  flp = (flp_t *) calloc (1, sizeof(flp_t));
  if(!flp)
    fatal("FLP: Memory allocation error\n");
  ...
  while(!feof(fp))		
	...
	{	
	  if (sscanf(copy, "%s%lf%lf%lf%lf", flp->units[i].name, &flp->units[i].width, \
	    ...
	    fatal("FLP: invalid floorplan file format\n");
	  i++;
	}	
    }
...
//----------------------------------------------------------------------------
//  free_flp : frees up the list containing the foorplan
...
int get_blk_index(flp_t *flp, char *name)
{
  ...
  if (!flp)
    fatal("FLP: Null pointer in get_blk_index\n");
  ...
  sprintf(msg, "FLP: Block %s not found\n", name);
  fatal(msg);
  return -1;
}
...
//----------------------------------------------------------------------------
//  is_horiz_adj : test for wether or not a block is adjancent in
...
//----------------------------------------------------------------------------
//  is_vert_adj : test for wether or not a block is adjancent in
