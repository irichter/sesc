...
void print_shape_entry(shape_t *shape, int i)
{
	...
	if (shape->left_pos)
		fprintf(stdout, "\t%d\t%d\t%.5f", shape->left_pos[i], 
	...
	if (node->label.unit >= 0) {
		...
		if (node->label.unit == CUT_VERTICAL) {
			...
			if(self->y[pos] > MAX(y1, y2)) {
				y1 = left->y[self->left_pos[pos]] += (self->y[pos] - MAX(y1, y2));
				y2 = right->y[self->right_pos[pos]] += (self->y[pos] - MAX(y1, y2));
			}	
			if(self->x[pos] > (x1+x2))
				x2 = right->x[self->right_pos[pos]] += self->x[pos]-(x1+x2);
			...
			if(self->x[pos] > MAX(x1, x2)) {
				x1 = left->x[self->left_pos[pos]] += (self->x[pos] - MAX(x1, x2));
				x2 = right->x[self->right_pos[pos]] += (self->x[pos] - MAX(x1, x2));
			}	
			if(self->y[pos] > (y1+y2))
				y2 = right->y[self->right_pos[pos]] += self->y[pos]-(y1+y2);
